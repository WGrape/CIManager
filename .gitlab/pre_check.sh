...
if [ "${APIDOC_SWITCH}" == "on" ]; then
  if [ "${APIDOC_TRIGGER_CMD}" == "" ] || [ "${APIDOC_FILE}" == "" ] ; then
    ...
    SendFailureNotice
    exit 1
  elif [ "${GITLAB_HOST}" == "" ] || [ "${GITLAB_API_TOKEN}" == "" ] || [ "${PROJECT_ID}" == "" ] ; then
    FAILURE_REASON="Failed to run pre_check.sh: the variables GITLAB_HOST/GITLAB_API_TOKEN/PROJECT_ID empty"
    SendFailureNotice
    exit 1
  fi
fi
